...
For example, we might want to remove rows if the age or school is missing, but we might not mind if their are other missing values (for example, we might expect some missing values in G1 if a student didn't receive a grade for whatever reason).
...
\tcblower
To remove rows that contain missing values or NA's in a specific column, we use the following code. Let's call our new dataset \verb|'student_results_rmNAage'| (rmNAage meaning remove NAs for age):
\begin{verbatim}
