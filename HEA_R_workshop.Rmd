...
-   Install git on your laptop: <https://git-scm.com/downloads> (optional);
...
If wanting to use git, you'll also need to make sure that git is set up in the git/SVN pane of global options in R-Studio (found in the Tools drop down menu).
...
Once you open a git-controlled repository, you'll get an extra panel, named 'git', in the top right pane of R-Studio and you'll also be able to use git in the 'Terminal' tab at the bottom left (in the same place as the R console).
...
To create vectors in R, we use `c()`.
...
Lists can contain heterogeneous data types such as vectors or another lists.
...
\end{tcolorbox}
```
**Matrices** are used in R to store 2-dimensional arrays.
...
\end{tcolorbox}
```
**Arrays** are n-dimensional arrays.
...
\end{tcolorbox}
```
**Data frames** are tabular data objects in R.
