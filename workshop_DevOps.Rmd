...
Now grab the repo url and replace \verb|<repo_url>| in the next command with the actual url from Azure DevOps:
...
To have something to work with, we need some data. There should be a data folder in the repository already, so all we need to do is grab some data and save it there. For this workshop, we'll use a data file from the [Key Stage 4 performance publication](https://explore-education-statistics.service.gov.uk/data-catalogue/data-set/462d625a-f233-4f16-98d9-eaed10014157) on Explore Education Statistics.
...
Download the data set ZIP file, extract the csv file and save it into the "data" folder in the repository (you can just use the normal Windows File Explorer to do this).
...
```
   	<<<<<<< HEAD
		...
		dfKS4 <- read.csv("data/202425_subject_national_gcse_exam_entriesgrades_timeseries_provisional.csv")
...
plot_ACrange_tidy(dfKS4tidied)
```
We've made a change to a file, so to fully resolve the conflict (and complete the merge), we'll need to perform and `add` and `commit`. Do that whichever way you prefer and at that point the `groupN_task1` branch will be fully merged into the `groupN_task3` branch.
