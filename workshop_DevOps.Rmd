...
Now we'll add in some simple code to read in the file. Create a script called `main.R` in the repository's root *directory* within your group's *branch* (`workshop_group_N`). The add the following code to it:
...
Let's do a quick commit to log that change. In the terminal run `git add .` and `git commit -m "Added new data and reading it in."`.
