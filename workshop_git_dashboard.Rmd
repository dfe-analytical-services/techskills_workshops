...
This task underpins the other two. You need to read in the data file that we've already downloaded. Start by creating a new branch: go to the BASH terminal, and type:
...
We've made a function to read in the data, but we're not running that function anywhere. So now open up *global.R* and add the following line at the end of the script:

`dfProgressHE <- featReadData()`

Give it a test, by sourcing *global.R* and checking the contents of `dfProgressHE` (e.g. by typing `dfProgressHE` in the R console). Once you're happy, then run another add/commit/push cycle and flag to your team that you've finished your task and the data read in is ready to use.

### Task 1b - Creating a chart

There's already an example chart in the template, so let's re-wire that as the basis for our demo chart here. Let's start by creating a ew branch in RStudio. With your repo open (and pulled with the latest changes) in RStudio, click the new branch button in the git panel toolbar. This will open a dialogue box as shown here in which you can enter a new branch name. Enter a name, make sure syncing to the remote is selected and press *Create*.
...
When your team mate from task 1 has completed their task, you'll want to merge their changes into your own branch, so that you have access to the data that's being read in:
...
| Process | git BASH                          | RStudio git panel                               |
|---------|-----------------------------------|-------------------------------------------------|
| Create new branch | `git checkout -b branch_name` | Click the new branch symbol |
| Switch to existing branch  | `git checkout branch_name` |                      |
| Merge    | `git merge branch_name`  | Not available in RStudio - merge by creating a pull request in GitHub/Dev Ops |
