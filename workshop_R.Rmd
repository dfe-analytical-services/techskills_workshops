...
In the bottom right you may also find a help tab. If not by typing "?" into the console followed by the function name it will search for functions with that name and bring it up in the help tab on the bottom right, from which you can navigate to their documentation, often including usage examples.

Similarly if you type "??" followed by a word, it will navigate to the help tab, but this time it will search the function name **and** documentation for that word, before providing a list of potential matches in the help tab.

# ADA and databricks

This workshop and it's materials were developed before the implementation of ADA or databricks, and while they are now becoming more commonplace in the department they remain out of scope for this introductory course. In due time will be incorporated into the workshop to account for new ways of working.
...
3.  Making an api call for example using **sparklyr** to access **databricks**.

Today we will cover 1. how to read in a **CSV** file but you will find optional sections on **SQL** and **databricks** towards the end.
...
Please download the HEA csv file used in the following example from: [HEA_some_student_results.csv](https://github.com/dfe-analytical-services/techskills_workshops/blob/main/data/HEA_some_student_results.csv) and can download it by clicking "More file actions" - the ellipsis on the same row as the file name. 
...
Note it is common to install the tidy package through installing the tidyverse package, which is a collection of useful packages such as dplyr and ggplot2.

a.  Install the `tidyr` or `tidyverse` packages,
...
> [(https://azure.microsft.com)](https://azure.microsoft.com/en-us/resources/cloud-computing-dictionary/what-are-large-language-models-llms)

[Copilot](https://copilot.microsoft.com/) is a new tool that is proving increasingly useful in learning to code, especially in it's capabilities of translating *natural language* to code.
...
There are still benefits to using Copliot, since if it gets the syntax right and a few commands wrong, you have the bulk of what you need and you can use Google to troubleshoot the mistakes.
...
Using Copilot
\tcblower
To start with, lets test out Copilot using the 'palmer penguins' data. This is an example dataset available in R that we can ask chatGPT about directly. Ask Copilot to do the following: 
...
Read the answer carefully, as alongside writing code for you, Copilot also provides explanations and commentary on the code it has written. Try copying and pasting the code output into a \textbf{new R code script} and running it on your device to test if it works!
\end{tcolorbox}
```
...
Then ask Copilot to create an R shiny app for you that displays a grouped bar chart for the assessment mean grades, grouped by sex, with a dropdown that filters on the time period.
Use Google to troubleshoot in the likely event that the code created doesn't work first time.

Top tips: You can define the data in Copilot by naming it and copying and pasting it as plain text (even if it's long!).
...
This is what we mean in our RAP guidance when we say [single dataset production script](https://dfe-analytical-services.github.io/analysts-guide/RAP/rap-statistics.html#dataset-production-scripts) or [whole publication prodcution script](https://dfe-analytical-services.github.io/analysts-guide/RAP/rap-statistics.html#dataset-production-scripts).
...
This error is usually due to Rtools not being properly installed, this should be uncommon since Rtools can be accessed from the Software Center.
...
Often it's not feasible to have all the data you want to work with stored on your local machine. This means that you have to have a way of pulling it down from external sources. Within the department we tend to use two forms of storage:
...
2.  Feed that connection to a wrapper which executes a piece of SQL in that area.

3.  (Depending on process) Collects the resulting output back to R.
...
```         
# Connect to SQL database for use with odbc, and DBI
...
```         
# Connect to SQL database for use with RODBC
...
a.  Use `DBI::dbGetQuery(con, SQL_string)`, with SQL_string being something like: `SQL_string = "select * from {SQL_table}"`

b.  i) Use `dplyr::tbl(con, in_schema(schema_name, {SQL_table}))`, or ii) `dbplyr::tbl(con,dplyr::sql(SQL_string)) %>% dplyr::collect()`, with SQL_string being something like: `SQL_string = "select * from {SQL_table}"`
...
## API calls, and accessing databricks data
One of the most common ways to transfer information online is via an application programming interface (API) call. An API call can be considered a client politely asking the server to do something.

Within R it is commonplace to use either the "httr" or "httr2" packages and you can see examples of use on EES e.g. [EES R api example](https://api.education.gov.uk/statistics/docs/reference-v1/endpoints/DownloadDataSetCsv/#example-request-py) and can try it out with the following I.D.: 18e39901-7fe3-8372-8b2b-33f7ae1e1d12

For working with databricks a different approach is used.

### Databricks

For databricks rather than trying to directly interact with the databricks REST API, if you have Java installed it may be better to follow the following guide:
[sparklyr](https://docs.databricks.com/aws/en/sparkr/sparkr-vs-sparklyr).

Otherwise you can use the Databricks REST API as above using the API commands detailed here: [Workspace access](https://learn.microsoft.com/en-us/rest/api/databricks/workspaces/get?view=rest-databricks-2024-05-01&tabs=HTTP#systemdata)
