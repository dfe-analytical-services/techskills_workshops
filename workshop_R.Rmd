...
For the file path, \textbf{if using git} you should make sure this is \textbf{not} saved in your OneDrive area (this is less important when you aren't using git). If using git you should select Browse and navigate out of OneDrive to save the project directory elsewhere (this is because OneDrive has it's own version-control system, and trying to use Git within that can cause complications and errors). 
...
3.  Making an API call, for example using the **sparklyr** package to access data stored in **Databricks**.

Today we will cover 1. Reading in a **CSV** file, but you will find optional sections on linking to **SQL** and **Databricks** towards the end.
...
When you're reading in the CSV file for the below example, make sure that you have saved it inside a folder called "data" (you may need to create this folder yourself), and that the file name matches exactly.
