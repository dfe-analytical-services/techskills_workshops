...
When creating RAP processes, it's key that you use code that is reusable, transparent, and well documented, such that it is future-proofed for any future team members. Therefore, first, we will show you how to create an R project. Using an R project ensures that all of the other files in the project folder (scripts, plots, notes) can all be referenced relative to the .Rproj file location. For example, if you have an R code script and a .csv file in a folder called 'workshop' that has been set up as an R project, with an .Rproj file, if you want to read the .csv into the R script, you don't need the entire file path of the csv, just the file path relative to the .Rproj file.

To create a new project, in R studio, go to File > New Project > New directory > New Project, then give your project a name and choose where abouts in your files you would like the project to be saved. Click 'Create project'.


## Using renv

Renv (short for R environment) is a package in R that helps you to keep a record of which packages your project uses, and what version of each package it uses. Using renv in your R project means that anybody in the future (including your future self) who comes back to this project, can immediately get all of the required packages. First you need to install the renv package by running install.packages('renv') in the console. 
...
When using functions from packages, you can either type 'the pacckage name'::'the function name' or just the function name on its own. However, problems can arrive if two packages have a function that uses the same name, so using the package name and colon is safer. 

To activate renv, run renv::activate() in the console. If you navigate back to the files tab and look in your project folder, you should see some renv-related files have appeared, including a renv.lock file. The renv.lock file is the file that records all of your project's packages and their versions. If you click on the renv.lock file it will open for you to view in the top left. 
...
Comments in code are some of the most useful documentation you can use, and are crucial for RAP! You should start your script with comments that provide important information such as what this code script contains and what it is for - basically anything you think a new person would need to know in order to understand and run the code effectively. You add comments in R scripts by starting the line with a hash symbol, #. Any line that starts with a hash symbol will be treated as a comment rather than code, and so will not be 'run'. You can also add comments to the end of lnes of code by including the hash symbol half-way through a line - everything after the # will be treated as a comment, and everything before will be treated as code. If you have multiple lines of comments you wish to add, you can type them out, highlight them and use ctrl+shift+C to 'comment out' all of the highlighted lines. 

Your first script should definitely include the library-calls for all of the packages you will need. 
