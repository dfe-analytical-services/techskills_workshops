...
In our environment, we should see the data we have loaded in with the name we gave it - 'student_results'. If we want to filter this, we could do so in the following format:
...
In today's workshop, we don't need to filter the data before we start aggregating, so we don't need to give it a name. The first thing we want to do is specify the variables we want to group the data by. In this example we want to group by **year, school, sex** and **age**. This is very easily done using the `group_by()` function as below: 
...
Provided you've created the student table on your SQL database using `dbWriteTable()` as described earlier, we can demo this now. In the SQL folder in your workshop repository, you'll find a simple SQL aggregation script that does the same aggregation as we've just done in R. First though we'll try just sending a very basic SQL query to the database:

`dbGetQuery(con, statement = "SELECT year, school, sex, age,
   G1, G2, G3  
   FROM dbo.sd_sandbox_student_results")`

Now we've seen how that works, we'll use a pre-built function that reads in the SQL script in a way that R can send it to the SQL server. This pre-built function is in the script R/utils.R and is called `getSQL()`, so first we'll need to source the utils.R script: `source("R/utils.R")`.

Let's run the getSQL() command with the example script as follows:

`query <- getSQL("sql/aggregate_students.sql")`

This creates a string variable called `query` containing the SQL script. We can now send this to the database using:
...
`student_results_aggregated_sql <- tbl(con, Id(schema = "dbo", table = 'sd_sandbox_student_results_aggregated'))`

(where this time we're explicitly defining the schema with the function `Id`).

You should now find the aggregated data contained within the variable `student_results_aggregated_sql`.
...
Create a new script called run_pipeline.R. Add some comments to introduce that this is the script that will source and run the entire pipeline. It is important that your run_pipeline script is **always** well documented. In a real life situation, here is where you will explain what each script is before you source it, what it does and why. 
...
source("main.R")
```
Then below, add a comment and source() for your functions script and plots script. 
