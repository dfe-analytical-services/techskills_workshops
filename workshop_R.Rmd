...
writeLines(c("\\resizebox{48mm}{!}{\\includegraphics{images/Department_for_Education.png}}\n",
             ...
             "\\raggedright\\HUGE{\\color{dfeheadingblue}\\textbf{Technical Workshop: Introduction to R for Reproducible Analytical Pipelines}} \n",
             paste0("\\huge{\\color{dfeheadingblue}\\textbf{Statistics Services Unit}}"),
...
We've prepared this walkthrough guide for analysts as an introduction to the ways in which coding in R can be used for Reproducible Analystical Pipelines (RAPs), creating functions for typical tasks that teams may come across.
...
RStudio is a piece of software, specifically an integrated development environment (IDE), that enables us to write code in the R coding language, while also providing a visual and interactive interface.
...
-   Bottom right - File explorer/plots/packages/help/viewer.
...
In the bottom right there is a help tab. By typing "?" into the console followed by a function name it will search for any functions with that name and display them in the help tab, from which you can navigate to documentation which often includes usage examples.

Similarly if you type "??" followed by a word, it will search the function name **and** documentation for that word, before providing a list of potential matches in the help tab.
...
Create project
\tcblower
Select \textbf{File > New Project > New directory > New Project}. Choose an appropriate name for your project folder.

There are two tick boxes: 'Create a git repository' and 'Use renv with this project'. We won't be covering Git in this workshop, but if you would like to use renv, tick the box and and follow the optional renv instructions as you go through the exercises.
...
If you didn't tick the 'Use renv with this project' box when creating your project, install the renv package by running `install.packages('renv')` in the console.
...
Install packages
\tcblower
Our first steps only require two packages, 'readr' and 'dplyr'. You can either install these by typing \verb|install.packages('readr')| and \verb|install.packages('dplyr')| one at a time in the console, or you can install both packages at once by running \verb|install.packages(c('readr', 'dplyr'))|. Then, remember to take a \verb|renv::snapshot()| to add these to the renv.lock file if you are using renv. 
\end{tcolorbox}
```
...
3.  Accessing data stored in **Databricks**.

Today we will focus on 1. Reading in a **CSV** file, but you will find an optional section on accessing remote data later on. This covers reading data from and writing data to **SQL**, as well a link to guidance on connecting to **Databricks** if you would like to learn more about that.

Before loading in any data, we should add a new section header!
...
Please download the CSV data file [HEA_some_student_results.csv](https://github.com/dfe-analytical-services/techskills_workshops/blob/main/data/HEA_some_student_results.csv); you can download it by clicking "More file actions" - the ellipsis on the same row as the file name. 

For the example below, create a folder named "data" within your project directory (i.e., the folder where your .Rproj file is located). Save the CSV file inside this data folder, and ensure the file name matches exactly.
...
In this case there we can use the `drop_na()` function from the tidyr package.

Note it is common to install the tidyr package through installing the tidyverse package, which is a collection of useful packages such as dplyr and ggplot2.

a.  Install the `tidyr` or `tidyverse` package,

b.  Add the library calls to the correct section of the initial script,

c.  Take a snapshot to update the renv.lock file if using renv.
...
Using the [online guidance](https://dfe-analytical-services.github.io/analysts-guide/statistics-production/ud.html#overview-of-ees-data-files), we can see that the following columns are required for EES: time_identifier, time_period, geographic_level, country_code (or geographic level equivalent), country_name (or geographic level equivalent).
...
Install this package, add it to your library calls and take a snapshot if you are using renv.
...
We had some practice earlier writing functions.
We can apply our function-writing skills in many areas, one of which is creating plots.
If you are creating **many similar plots**, it can be really useful to create a function to save you repeating the same code over and over again!

Take the code above that we used to plot assessment against mean grade.
Imagine we want to compare the mean grade not just by sex, but also want to see comparisons for age, year, and school.
We can write a function to create a chart in which we simply input the data and the variables we want to split by each time, removing the need to repeat *all* of the plot code multiple times!
...
\begin{verbatim}
plot_function <- function(data, filteryear, filterage, 
                          filterregion, mapping){
  plot_data <- data %>%
    filter(time_period == filteryear, age == filterage, 
...
You can now choose the filters and mapping as you wish!
\end{verbatim}
\end{tcolorbox}
```
...
> Copilot is a large language model chatbot developed by Microsoft AI.
...
> [(https://azure.microsoft.com)](https://azure.microsoft.com/en-us/resources/cloud-computing-dictionary/what-are-large-language-models-llms)

[Copilot](https://copilot.microsoft.com/) is a new tool that is proving increasingly useful in learning to code, especially in its capabilities of translating *natural language* to code.
This means if you know what you would like to do and can express that in your natural, spoken language, Copilot is pretty capable of translating that into code syntax for you.
...
There are still benefits to using Copliot, since if it gets the syntax right and a few commands wrong, you have the bulk of what you need so it saves time, and you can use Google to troubleshoot the mistakes.
...
Using Copilot
\tcblower
Let's test out Copilot using the 'palmer penguins' data. This is an example dataset available in R that we can ask Copilot about directly. Ask Copilot to do the following: 

\begin{verbatim}
"Write some R code that will calculate the % of penguins in the 
palmer penguins data that have a body mass greater than 4500"
\end{verbatim}

Read the answer carefully, as alongside writing code for you, Copilot also provides explanations and commentary on the code it has written. Try copying and pasting the code output into a \textbf{new R code script} and running it to test if it works!
\end{tcolorbox}
```
Now we can test out something a bit harder using the student results data we have been working with.
First of all, recreate your plot_data without filtering on time period so that it contains rows for multiple years.
Then ask Copilot to create an R Shiny app for you that displays a grouped bar chart for the assessment mean grades, grouped by sex, with a drop-down that filters on the time period.
Use Google to troubleshoot in the likely event that the code created doesn't work first time.

Top tips: You can attach files to a Copilot prompt!

```         
Write R code for a Shiny app that....
```

If the output code suggests you save the plot_data as a CSV and then read it in, you can either do that to practice this, or comment out this line since plot_data will already be defined in your environment from earlier in the workshop.

If you have any specific or more advanced questions that we haven't covered in this workshop, we encourage you to try and use Copilot for these in any additional time you may have, as well as of course asking the workshop facilitators to support!
...
This is what we mean in our [RAP guidance](https://dfe-analytical-services.github.io/analysts-guide/RAP/rap-statistics.html#dataset-production-scripts) when we say single dataset production script or whole publication production script.
We don't mean that *all* of your code is in one single script, but instead that there exists a single script (called run_pipeline.R here) that will *source* each of your scripts, in order.
...
## Unit testing in R (optional)

A unit test is basically a small function that tests and helps us to write *robust* code.
By robust, we mean code that **won't break easily** upon changes, can be altered or extended, and can be tested with ease.
Unit tests are extremely useful when writing **dynamic** code, as there isn't always a straight-forward way to check and test functions without 'invalid argument' errors!

A simple function takes some input x and returns an output y.
In unit testing, we verify that the function is returning the \*\*expected value\* of y for a specific value of x!
We can perform unit tests using the `testthat` package.
Make sure you install the `testthat` package first!
...
## Accessing remote data (optional)

Often it's not feasible to have all the data you want to work with stored on your local machine. This means that you have to have a way of pulling it down from external sources. Within the department we are currently using two forms of external storage:

1. On-site SQL servers.
2. Databricks lakes, lakehouses and warehouses.

### SQL

**NOTE:** to work through the tasks in these SQL sections, you must have access to an existing SQL database. We will use {SQL_server}, {SQL_database}, and {SQL_table} instead of your server, database, and table names throughout.

Most methods of interacting with an SQL database in R follow the below structure:

1.  Create a connection to the SQL database of interest,

2.  Feed that connection to a wrapper which executes some SQL code in that area,

3.  (Depending on process) Collect the resulting output back in R.

In order to read data from, and write data to, a SQL database we need to install the relevant packages and connect to that SQL database.

To apply the various options in this guide you should;

a.  Install the `odbc`, `DBI`, `RODBC`, and `dbplyr` packages,
...
```         
# Connect to SQL database for use with odbc and DBI packages
con_DBI <- DBI::dbConnect(odbc::odbc(),
    Driver = "SQL Server",
    Server = "{SQL_server}",
...
```         
# Connect to SQL database for use with RODBC package
con_RODBC <- RODBC::odbcDriverConnect(
          paste0(
            "driver = SQL Server",
            "server =",{SQL_server},
            "database =",{SQL_database},
...
**[Note that this code will only work for you if you have access to the specified server and database! If someone without access tried to run this code, they would get an error in the R console.]**
...
Here we'll show the 3 possible ways of writing a table with our students dataframe:

a.  DBI's dbWriteTable: `dbWriteTable(con_DBI,  Id(schema = "dbo", table = "sd_sandbox_student_results"), student_results)`

b.  dbplyr's db_copy_to: `db_copy_to(con_DBI,  table = "sd_sandbox_student_results", student_results, overwrite = TRUE)`

c.  RODBC's sqlSave: `sqlSave(con_RODBC, student_results, tablename = "sd_sandbox_student_results")`
...
\tcblower
If you have existing write access to a SQL database that you or your team owns, you can give the above code a go to write the student results data that we have just pulled in to your SQL database, as long as you defined your connection object using the server and database name. 
...
As well as writing tables from R to the database, we can use our connection to read in a table from the database into R.
There are multiple ways to do this:

a.  Use `DBI::dbGetQuery(con, SQL_string)`

b.  Use i) `dplyr::tbl(con, in_schema(schema_name, {SQL_table}))`, or ii) `dbplyr::tbl(con,dplyr::sql(SQL_string)) %>% dplyr::collect()`

c.  Use `RODBC::sqlQuery(con, "select * from {SQL_table};")`

with `SQL_string` being SQL code to query data from the database - something like: `SQL_string = "select * from {SQL_table}"`

**[Note that schema refers to a structural component of an SQL_database used to better organise tables.]**

Option b.i) above is a very concise and tidy way to read in data from SQL tables, which (as long as you've added descriptive comments) will make your code easy to read and understand.
...
**[Note that while slower option b.ii) enables standard dplyr manipulation before collection, saving you local resourcing e.g. tbl(...) %>% mutate(...) %>% group_by(...) %>% summarize(...) %>% collect(...)]**

Option c. is slightly longer to write and takes much longer to run, but it does allow you to write SQL queries directly in R, so if you have previous experience with SQL you may find this useful.
...
The data we are using today contains data on the characteristics and grades of some students, so we might choose to call the table 'student_results'.
We assign the table this name by using the `<-` symbols, so the full snippet would look something like this:
...
If you managed to write the table to your SQL database in the above section, add this code under an appropriate heading/comment to your initial script.
Then, if you navigate to the 'environment' window in the top right of the screen, you should see an object has appeared with your chosen name under the heading 'Data'.
If you click the blue circle with the white arrow inside, it will expand to show you information about the data you've uploaded, including the name of every column/variable, and the format of each column (i.e. chr for character, num for numeric).
If you click the name of the object, it will open in the viewer in the top left so that you can look at the table.
...
[This reference table](https://dfe-analytical-services.github.io/analysts-guide/learning-development/r.html#sql-function-equivalents-in-r) shows how you would carry out some popular SQL commands in R.
...
[View the reference table on our guidance site.](https://dfe-analytical-services.github.io/analysts-guide/learning-development/r.html#sql-function-equivalents-in-r)

### Alternative ways to read in data

There are other ways to read in data to RStudio, for example by creating a connection to Databricks or by using an application programming interface (API) call. Neither of these are covered in this introductory workshop but both are used within the Department and we would strongly recommend taking time to learn about these options.

You can find guidance on getting started with Databricks [here](https://dfe-analytical-services.github.io/analysts-guide/ADA/databricks_fundamentals.html).
...
This error is usually due to RTools not being properly installed, which should be uncommon since Rtools can be accessed from the Software Center.

If re-installing from the Software Center does not work then there is a work around:
...
When downloading RTools you may need to confirm that you understand the risks of downloading an .exe file from an unknown source; this is common, and as this is a trusted source, you can click accept.
...
R can do much of what you can do in Excel, but analysis done using R is less prone to human error.
[This reference table](https://dfe-analytical-services.github.io/analysts-guide/learning-development/r.html#excel-functions-in-r) shows how you would carry out popular Excel commands in R.
...
[View the comparison table on our guidance site.](https://dfe-analytical-services.github.io/analysts-guide/learning-development/r.html#excel-functions-in-r)
