...
student_results_aggregated %>%
  select(school, sex, age, students, G1_mean, G2_mean, G3_mean) %>%
  rename(PICK UP FROM HERE)
```


Since we have started the first line with `student_results_aggregated <-`, this means everything that follows will overwrite what was stored under this name before! This *can* be risky, since `select()` can also be used to **drop** columns. If you left out any names from the select line, they would simply be dropped, so be careful if you use the same name as the object from before this step! This can be avoided by picking a different name, like `student_results_aggregated_reordered` for example. 
